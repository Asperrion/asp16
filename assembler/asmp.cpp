// ПРИМЕЧАНИЕ!
// Ассемблер почти на фулл завайбкоден)))
// Было трудно такое самому реализовать, что то писалось мной, что то чинилось чатом гпт 5.2 (создавайте issues буду фиксить если что)
...
for (auto& ins : program) {
    ...
    // =====================
        ...
        emit(ins.op == "call" ? OP_CALL : OP_JMP,
             addr & 0xFF, addr >> 8, 0);
    }
